...
*                                                 CONSTANTE
**********************************************************************************/
	const    AP 		= 	-3	; //Volt command of the Kepco
	const    P 			= 	3	; //Volt command of the Kepco
	const    ZERO 		= 	0	; //Volt command of the Kepco
	const    MAX_I 		= 	1.00/1000	; //A
	const    MAX_V 		= 	1.00	; //V
...
// preparation (without measurement) fieldStart -> fieldFinal
	...
	const    currentStart 	=	-0.1	; //mA
	const    currentStop	=	-0.1	; //mA	
	const    currentInterval=	-1	; //mA

	const    technician		= "Sylvain MARTIN"	
	const    serieName		= "LS14W18"	
	const    sampleName		= "L1";
	const    junctionLineName	= "L1";
	const    junctionColName	= "-";
	const    junctionTypeName    = "02";
	
	const    junctionName		=  "L1"; //junctionColName + 
						//junctionLineName + 
						//junctionTypeName;
...
function oneFieldPoint(data_, plot_, DAQ, H) {
			...
			delay(0.10); // because without that, the field is not stable
