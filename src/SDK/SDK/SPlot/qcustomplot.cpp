...
*/
void QCPGraph::setData(QCPDataMap *data, bool copy,  bool bijective)
{
  if (copy)
  {
    *mData = *data;
...
/*! \overload
  ...
  smallest vector.
*/
void QCPGraph::setData(const QVector<double> &key, const QVector<double> &value, bool bijective)
{
  mData->clear();
  int n = key.size();
  n = qMin(n, value.size());
...
/*! \overload
  ...
  vectors should have equal length. Else, the number of added points will be the size of the
  smallest vector.
  the parameter bijective has to be false if your data represent a function that can have 
  more than one y value or an x value. By example a R(H) curve.

  if bijective is false, the point will be link in the order of the data otherwise
  the data will be placed in order of x.contains by default bijective is false.

  A bijection (or bijective function or one-to-one correspondence) is a function 
  giving an exact pairing of the elements of two sets. Every element of one set 
  is paired with exactly one element of the other set, and every element of 
  the other set is paired with exactly one element of the first set.

  \remark add by Sylvain Martin for SpinToolBox
*/
void QCPGraph::setData(const QList<double> &key, const QList<double> &value, bool bijective)
{
	mBijective = bijective;
	...
	for (int i=0; i<n; ++i)
	{
		newData.key = key.at(i);
		newData.value = value.at(i);
		mData->insertMulti(bijective?newData.key:i, newData);
	}
...
/*! \overload
  ...
  if bijective is false, the point will be link in the order of the data otherwise
  the data will be placed in order of x
...
if(mBijective){ 
	...
	QCPDataMap::iterator i = dataMap.begin();

	while (i != dataMap.end()) {
		mData->insertMulti(p, i.value());
			p++	;
	}
...
{
	mData->insertMulti(mBijective?data.key:mData->count(), data); // change by Sylvain Martin
}
...
void QCPGraph::addData(double key, double value)
{
  ...
  newData.key = key;
  newData.value = value;
  mData->insertMulti(mBijective?newData.key:mData->count(), newData); // change by Sylvain Martin
}
...
void QCPGraph::addData(const QVector<double> &keys, const QVector<double> &values)
{
  ...
  int shift = mData->count();
  for (int i=0; i<n; ++i)
  {
	  newData.key = keys.at(i);
	  newData.value = values.at(i);
	  int p =i+shift;
	  mData->insertMulti(mBijective?newData.key:(i+shift), newData);
  }
}
...
/*! \overload
  ...
  \see removeData
*/
void QCPGraph::addData(const QList<double> &keys, const QList<double> &values)
{
  int n = qMin(keys.size(), values.size());
  QCPData newData;
  int shift = mData->count();
...
/*!
  ...
  \remark Modified by Sylvain MARIN to be compatible with the non-Bijective functions
  \see addData, clearData
*/
void QCPGraph::removeDataAfter(double key)
{
	 if (mData->isEmpty()) return;
	QCPDataMap::iterator i = mData->begin();
...
/*!
  ...
  \remark Modified by Sylvain MARIN to be compatible with the the non-Bijective functions
  \see addData, clearData
*/
...
void QCPGraph::getLinePlotData(QVector<QPointF> *lineData, QVector<QCPData> *pointData) const
{
  ...
  if (pointData)
	...
	while (it != upperEnd)
	{
		...
		(*lineData)[i].setY(mKeyAxis->coordToPixel(it.value().key));
		++i;
		++it;
	}
	} else // key axis is horizontal
	...
	while (it != upperEnd)
	{
		if (pointData)
		(*pointData)[i] = it.value();
		(*lineData)[i].setX(mKeyAxis->coordToPixel(it.value().key));
...
// 	si dans les data les key ne sont pas ordonnées, alors, 
// 	une partie des points seront traité pour rien
// 	mais on n'a pas trop le choix car sinon, on ne 
// 	peut pas traiterter les fonction non bijective (ce qui
// 	n'est pas tres pratique pour faire des R(H)).
	QCPDataMap::const_iterator lbound;
	QCPDataMap::const_iterator ubound;

// get visible data range as QMap iterators
	if (mBijective){
		lbound = mData->lowerBound(mKeyAxis->range().lower);
		ubound = mData->upperBound(mKeyAxis->range().upper)-1;
		
	}else{
		//Hack a la con, pour les fonctions non bijective, on prend tout
		lbound = mData->begin();
		ubound = mData->end()-1;
	}
