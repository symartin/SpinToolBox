...
class SPlot2D; // Add by Sylvain MARTIN for SpinToolBox.
class QCustomPlot;
class QCPLegend;
class QCPRange;
class QCPAxis;
...
{
  Q_OBJECT
  friend class SPlot2D; // Add by Sylvain MARTIN for SpinToolBox.
...
public:
  ...
  */
  ...
  bool bijective(){return mBijective;}
  
  // setters:
  void setData(QCPDataMap *data, bool copy=false, bool bijective = false); // bijective add by Sylvain Martin
  void setData(const QVector<double> &key, const QVector<double> &value, bool bijective = false);// bijective add by Sylvain Martin
  void setData(const QList<double> &key, const QList<double> &value, bool bijective = false); // add by Sylvain Martin
  void setData(const QList<double> *key, const QList<double> *value, bool bijective = false); // add by Sylvain Martin
  ...
  void addData(const QList<double> &keys, const QList<double> &values);
  void addData(const QList<double> *keys, const QList<double> *values);
